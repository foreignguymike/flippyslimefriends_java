package com.distraction.fs2j.tilemap.data;

import com.badlogic.gdx.graphics.Color;

public class GameColor {
    // palette is Journey by PineappleOnPizza
    // downloaded from https://lospec.com/palette-list/journey

    // slime colors
    public static final Color LIME_GREEN = new Color(198 / 255f, 216 / 255f, 49 / 255f, 1f);
    public static final Color BLUE = new Color(72f / 255f, 139f / 255f, 212f / 255f, 1f);
    public static final Color YELLOW = new Color(1f, 245f / 255f, 64f / 255f, 1f);
    public static final Color CYAN = new Color(176 / 255f, 255 / 255f, 241 / 255f, 1f);
    public static final Color VIOLET = new Color(1f, 65f / 255f, 125f / 255f, 1f);
    public static final Color SKY_BLUE = new Color(120f / 255f, 215 / 255f, 1f, 1f);
    public static final Color PURPLE = new Color(148f / 255f, 0f, 122f / 255f, 1f);
    public static final Color ORANGE = new Color(255 / 255f, 184 / 255f, 74 / 255f, 1f);
    public static final Color LIGHT_GRAY_PURPLE = new Color(199 / 255f, 212 / 255f, 225 / 255f, 1f);
    public static final Color ORANGE_RED = new Color(237f / 255f, 76f / 255f, 64f / 255f, 1f);
    public static final Color PINK = new Color(1f, 122f / 255f, 125f / 255f, 1f);

    // kind of slime colors
    public static final Color TAN = new Color(207f / 255f, 150f / 255f, 140f / 255f, 1f);

    public static final Color RED_ORANGE = new Color(212f / 255f, 100f / 255f, 83f / 255f, 1f);
    public static final Color GREEN = new Color(119 / 255f, 176 / 255f, 42 / 255f, 1f);
    public static final Color RED = new Color(212f / 255f, 30f / 255f, 60f / 255f, 1f);
    public static final Color TEAL = new Color(16f / 255f, 144f / 255f, 142f / 255f, 1f);
    public static final Color DARK_GREEN = new Color(66 / 255f, 144 / 255f, 88 / 255f, 1f);
    public static final Color DARK_TEAL = new Color(21f / 255f, 60 / 255f, 74 / 255f, 1f);
    public static final Color MIDNIGHT_BLUE = new Color(5 / 255f, 9 / 255f, 20 / 255f, 1f);
    public static final Color DARK_BLUE = new Color(26f / 255f, 70f / 255f, 107f / 255f, 1f);
    public static final Color PEACH = new Color(255 / 255f, 207 / 255f, 142 / 255f, 1f);
    public static final Color BLACK_1 = new Color(5f / 255f, 9f / 255f, 20f / 255f, 1f);
    public static final Color BRIGHT_YELLOW = new Color(248 / 255f, 255 / 255f, 184 / 255f, 1f);
    public static final Color NEON_GREEN = new Color(61 / 255f, 255 / 255f, 110 / 225f, 1f);
    public static final Color SAND = new Color(240 / 255f, 194 / 255f, 151 / 255f, 1f);
    public static final Color DARK_BURGUNDY = new Color(115 / 255f, 38 / 255f, 61 / 255f, 1f);

    // palette is Optimized Grayscale 4 Palette by Yousurname
    // downloaded from https://lospec.com/palette-list/optimized-grayscale-4
    public static final Color VERY_DARK_GRAY = new Color(32 / 255f, 32 / 255f, 32 / 255f, 1f);
    public static final Color DARK_GRAY = new Color(96 / 255f, 96 / 255f, 96 / 255f, 1f);
    public static final Color GRAY = new Color(159 / 255f, 159 / 255f, 159 / 255f, 1f);
    public static final Color LIGHT_GRAY = new Color(223 / 255f, 223 / 255f, 223 / 255f, 1f);

    public static final Color BLACK = new Color(0f, 0f, 0f, 255f);
    public static final Color WHITE = new Color(1f, 1f, 1f, 1f);
}
